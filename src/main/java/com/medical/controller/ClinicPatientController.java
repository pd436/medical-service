...
public class ClinicPatientController {
    ...
    public ResponseWrapper<Integer> deletePatientIllness(@RequestParam(name = "patientId") String patientId, @RequestParam(name = "illnessId") String illnessId) {
        System.out.println(patientId);
        System.out.println(illnessId);
        Integer result = this.patientService.deleteIllnessForPatient(Integer.valueOf(patientId), Integer.valueOf(illnessId));
    ...
    @PostMapping(value = ResourceReference.PATIENT_APPT_ADD, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseWrapper<Consultation> savePatientAppointment(@RequestBody Consultation consultation) {
        Consultation result = this.consultationService.addPatientConsultation(consultation);
        return new ResponseWrapper<>(new Metadata(true, "Saves appointment for patient"), result);
    }

    @PostMapping(value = ResourceReference.PATIENT_APPT_REMOVE, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseWrapper<Consultation> removePatientAppointment(@RequestParam(name = "consultationId") String consultationId) {
        this.consultationService.removePatientConsultation(Integer.valueOf(consultationId));
        return new ResponseWrapper<>(new Metadata(true, "Removes appointment for patient"), null);
    }
